...
export async function getFriends(req,res){
    try{
        const user = User.findById(req.user.id).select("friends").populate("friends","fullName profilePic nativeLanguage learningLanguage");
...
export async function sendFriendRequest(req,res){
    try{
        ...
        const reciepient = await User.findById(recipientId);
        if(!reciepient){
            return res.status(404).json({message:"Recipient user not found"});
        };

        if(reciepient.friends.includes(myId)){
...
export async function acceptFriendRequest(req,res){
    try{
        ...
        // addToSet prevents duplicates : adds if not already present
...
export async function getFriendRequests(req,res){
    try {
        ...
        const acceptedReqs = await FriendRequest.find({
            sender:req.user.id,
            status:"accepted"
        }).populate("reciepent","fullName profilePic");
